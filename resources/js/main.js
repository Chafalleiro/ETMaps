...
var fileName, myVersion = "";
var currentVersion = "0.2.1";
let arr_curr,arr_new = "";
var arrPlanets = {
	planet:'', arrAnt:[], arrNew:[]
};
var swFile, swFile1, swFile2 =false;
let lst_planets;
let listClassAnt  = [];
prevLenght = 0;
let varDemo = "";
...
let readDatafileNew = async (fileNew) => {
								let responseNew = await Neutralino.filesystem.readFile(fileNew);
									let dataNew = await responseNew;
									 arr_new= dataNew.split("\r\n");
							console.log("readDatafileAnt "+fileNew);
							console.log("arr_curr "+arr_curr[0]);
							console.log("arr_new "+arr_new[0]);
								}
function main()
{
...
//Open a file for reading
	...
	//crappy wait for data to arrive
	swFile1 = false;
	readDatafile(fileName,"myVersion");
    var instanceVr = window.setInterval(function() {
		if(swFile1){
			window.clearInterval(instanceVr);
			if (!myVersion || myVersion !== currentVersion)
				{
				statusdisplay("EtMaps v"+myVersion+" found updating to v"+currentVersion+", merging datafiles...");
				...
				for(i=1;i < arr_planets.length;i++)
					...
					for(j=0;j < lst_planets.length;j++){
						if (lst_planets[j] != "")
							{								
							var fileAnt = NL_CWD+'/data/' + lst_planets[j] + '.csv';
							var fileNew = 'data/' + lst_planets[j] + '.csv';
						console.log("readDatafileAnt "+fileAnt);
							let readDatafileAnt = async () => {
								
								let response = await Neutralino.filesystem.readFile(fileAnt);
									let data = await response;
									 arr_curr= data.split("\r\n");
								console.log("readDatafileAnt "+fileAnt);
								readDatafileNew(fileNew);
								}
								readDatafileAnt();
/*							var instanceFl = window.setInterval(function() {
								if(swFile2)
									{
									console.log("arr_curr "+arr_curr);
									window.clearInterval(instanceFl);
									console.log("Planet "+lst_planets[j]+ " 1st entry "+arr_curr[1]);
									swFile2 = false;
									}
								},10)
*/
				...
				console.log("EtMaps Version: " + myVersion);statusdisplay("EtMaps Version up to date: " + myVersion);
				}
			}
		},10)
}
...
let writeWaypoints = async (data) =>
{
	...
	Neutralino.filesystem.writeFile(file,dataWay,
		function (data){statusdisplay("writing Waypoints in planet: "+ document.getElementById("inputMap").value)},
...
{
	planet = document.getElementById("inputMap").value;
	swFile=false;
	...
	document.getElementById("chngMap").value = planet;
	changemap(planet);
//crappy half assed semaphore that waits for an await function to end
    var instanceCr = window.setInterval(function() {
		if(swFile){
			window.clearInterval(instanceCr);
			arr_waypoints.push(dataWay);
			writeWaypoints(planet);
...
//************************ Waypoints
let readDatafile = async (datafile,dataArray) => {
let response = await Neutralino.filesystem.readFile(datafile);
	let data = await response;
...
let response = await Neutralino.filesystem.readFile(NL_CWD+'/data/' + datafile + '.csv');
	let data = await response;
...
function createMapLabels()
{
	...
	let div_overlay,div_overlay_img,listWaypoints,img_eye,div_content,caret;
	let table_waypoints;
	let table_row;
	...
	for (let i = 1; i < arr_waypoints.length; i++)
	{
		...
		else //Create a li and a table for a menu with accordions.
		{
			...
			caret = document.createElement("img");
			caret.src = "icons/sort-w.png";
			caret.title =  "Toogle " + coords[4] + " list order";
			caret.style.cursor = "pointer";
			caret.style.cssFloat = "right";
			caret.id = "sortClass_"+coords[4];
			lbl_listWaypoints.appendChild(caret);
			div_content.appendChild(table_waypoints);
			li_listWaypoints.appendChild(div_content);

			caret.addEventListener("click", function(e){sortTable(1,coords[4]);e.preventDefault();}, false);			
		...
		img_cell.src = "icons/eye-symbol-16.png";
...
function switchClass(myClass)
{
	...
	{
		document.getElementById("eyeClass_"+myClass).src = "icons/eye-symbol-w-16.png";
	...
	else
	{
		...
		document.getElementById("eyeClass_"+myClass).src = "icons/eye-crossed-w-16.png"
...
if (document.getElementById("ChkVis_" + myCoords[4]).checked === false)
	...
	if (document.getElementById("chk_"+myWaypoint).checked === false)
		...
		document.getElementById("eye_"+myWaypoint).src = "icons/eye-symbol-16.png"
		...
		document.getElementById("eye_"+myWaypoint).src = "icons/eye-crossed-16.png"
...
//Sort tables
//Modified example from https://www.w3schools.com/
function sortTable(n,myTable) {
  var table, caret, rows, switching, i, x, y, shouldSwitch, dir, switchcount = 0;
  table = document.getElementById("table_" + myTable);
  caret = document.getElementById("sortClass_" + myTable);
  switching = true;
    ...
    for (i = 0; i < (rows.length - 1); i++)
		...
		if (dir == "asc")
			...
			if (x.innerHTML.toLowerCase() > y.innerHTML.toLowerCase())
				...
				caret.src = "icons/caret-up-w.png";
				shouldSwitch= true;
				break;
				}
			} else if (dir == "desc")
				...
				if (x.innerHTML.toLowerCase() < y.innerHTML.toLowerCase()) {
				//if so, mark as a switch and break the loop:
				caret.src = "icons/caret-down-w.png";
				shouldSwitch = true;
				break;
				}
			}
